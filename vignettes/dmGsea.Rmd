...
In some omics datasets, such as Illumina DNA methylation data, genes are often represented by variable 
number of correlated probes, and a single probe can map to multiple genes. This complex data structure 
poses significant challenges for gene set enrichment analysis. The `r Biocpkg("dmGsea")` package offers several  
functions with novel methods specifically designed to address probe dependency and probe number bias. 
...
<ul>
<li>`gsGene()`:  Gene set enrichment analysis based on gene level p-values</li>
<li>`gsPG()`:  Gene set enrichment analysis based on probe group p-values</li>
<li>`gsProbe()`:  Gene set enrichment analysis based on probe level p-values</li>
<li>`gsRank()`:  Fast enrichment analysis based on ranks of gene level statistics</li>
</ul>
