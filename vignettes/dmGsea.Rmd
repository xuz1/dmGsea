...
In many omics datasets, such as Illumina DNA methylation data, genes are often represented by variable 
number of correlated probes, and a single probe may map to multiple genes. This complex data structure 
poses challenges for gene set enrichment analysis. The `r Biocpkg("dmGsea")` package provides both 
threshold- and ranking-based competitive enrichment analysis methods to address probe correlation 
and probe number bias. These functions are versatile and can be applied to various omics datasets, 
such as DNA methylation, proteomics and gene expression data.
...
## Example 4: Enrichment analysis with user provided geneset
...
## Example 5: Enrichment analysis for gene expression type of data that do not need to combine test statistics
